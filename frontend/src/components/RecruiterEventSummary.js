...
const RecruiterEventSummary = ({ recruiterId }) => {
  const { data, isLoading } = useQuery(
    ['recruiter-events', recruiterId],
    () => fetch(`/api/recruiters/${recruiterId}/events`).then(res => res.json()),
    {
      enabled: !!recruiterId,
      staleTime: 1000 * 60 * 5
    }
...
export default RecruiterEventSummary;
