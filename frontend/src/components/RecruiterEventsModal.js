...
const EVENT_OPTIONS = [
  { value: 'contact', label: 'Recruiter Contact' },
  { value: 'resume_request', label: 'Requested Resume' },
  { value: 'follow_up', label: 'Follow Up' },
  { value: 'note', label: 'General Note' }
];

const RecruiterEventsModal = ({ recruiter, onClose }) => {
  ...
  const { data: eventsData, isLoading } = useQuery(
    ['recruiter-events', recruiter?.id],
    () => fetch(`/api/recruiters/${recruiter.id}/events`).then(res => res.json()),
    { enabled: !!recruiter?.id }
  );

  const addEventMutation = useMutation(
    async (payload) => {
      const response = await fetch(`/api/recruiters/${recruiter.id}/events`, {
        method: 'POST',
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['recruiter-events', recruiter.id]);
  ...
  const deleteEventMutation = useMutation(
    async (eventId) => {
      const response = await fetch(`/api/recruiter-events/${eventId}`, {
        method: 'DELETE'
      });
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Contacted me about cloud role"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  placeholder="Add any context or follow up details"
                  rows={4}
                  className="form-input"
          ...
          <div style={{ padding: '24px', overflowY: 'auto' }}>
            ...
            {isLoading ? (
              ...
              <div style={{
                ...
                No events yet. Log your first recruiter touch point.
              </div>
            ) : (
...
export default RecruiterEventsModal;
